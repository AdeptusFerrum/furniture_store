create table if not exists transit_status(
    transit_status_id serial not null constraint pk_transit_status_id,
...
create table if not exists Model_Automobile(
    Model_Automobile_id serial not null constraint pk_model_automobile_id,
    Model_automobile_name varchar(50) not null 
);
create table if not exists color_automobile(
    color_automobile_id serial not null constraint pk_color_automobile_id,
    color_automobile_name varchar(36) not null
);

create table if not exists mark_automobile(
    mark_automobile_id serial not null constraint mark_automobile primary key,
...
create table if not exists Users(
    ...
    users_login varchar(36) not null  not null,
...
create table if not exists Product_Avaliable(
    ...
    Product_Avaliable int not null
);
...
create table if not exists Defective_Elements(
    Defective_Elements serial not null constraint pk_defective_elements_id primary key,
...
create table if not exists Employee_Post(
    Employee_Post_id serial not null constraint pk_delivery_id primary key,
...
create table if not exists customers_org(
    customers_org_id serial not null constraint pk_organizations_id primary key,
    legal_form_id int not null references Legal_Form(legal_form_id),
...
create table if not exists agent_post(
    agent_post_id serial not null constaint pk_agent_post_id primary key,
    agent_post_name varchar(50) not null
);

create table if not exists agent_auth (
    agent_auth_id serial not null constraint pk_agent_auth_id,
...
create table if not exists Order_Check(
    ...
    Order_Check_Number varchar(14) not null constraint unique UQ_Order_Check,
...
create table if not exists Order_Content(
    ...
    order_content_price decimal(12,2)
);

create table if not exists country_automobile(
    country_automobile_id serial not null constraint pk_country_automobile_id primary key,
    country_automobile_name varchar(3) not null
);
...
create table if not exists region_automobile(
    ...
    region_automobile_number int varchar(6)
);
...
create table if not exists automobile(
    ...
    country_automobile int not null references country_automobile(country_automobile_id),
